...
export default function TeacherRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/v1/auth/register/teacher`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        throw new Error(data.detail || "教师注册失败");
      }

      alert("教师注册成功，请登录");
      router.push("/login");
    } catch (err: any) {
      setError(err.message || "教师注册失败");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="max-w-2xl mx-auto bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        <h1 className="text-2xl font-bold text-slate-900 mb-6">教师注册</h1>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input
            ...
            onChange={(e) => updateField("teacher_no", e.target.value)}
            placeholder="工号"
            className="px-4 py-3 rounded-xl border border-slate-300 outline-none"
          />
          <input
